...
static immutable real MAXGAMMA = 1755.5483429L;

private {

static immutable real SQRT2PI = 2.50662827463100050242E0L; // sqrt(2pi)

// Polynomial approximations for gamma and loggamma.

static immutable real[] GammaNumeratorCoeffs = [ 1.0,
...
static immutable real[] GammaDenominatorCoeffs = [ 1.0,
...
static immutable real[] GammaSmallCoeffs = [ 1.0,
...
static immutable real[] GammaSmallNegCoeffs = [ -1.0,
...
static immutable real[] logGammaNumerator = [
...
static immutable real[] logGammaDenominator = [
...
real gammaStirling(real x)
{
    ...
    static immutable real[] SmallStirlingCoeffs = [
    ...
    static immutable real[] LargeStirlingCoeffs = [ 1.0L,
...
real logGamma(real x)
{
    ...
    static immutable real LOGSQRT2PI  =  0.91893853320467274178L; // log( sqrt( 2*pi ) )
...
private {
    ...
    static immutable real MINLOG = -0x1.6436716d5406e6d8p+13L; // log(real.min*real.epsilon) = log(smallest denormal)
    static immutable real BETA_BIG = 9.223372036854775808e18L;
    static immutable real BETA_BIGINV = 1.084202172485504434007e-19L;
}
...
real betaDistExpansion1(real a, real b, real x )
{
    ...
    r = 1.0L;
    n = 0;
    static immutable real thresh = 3.0L * real.epsilon;
    do  {
        xk = -( x * k1 * k2 )/( k3 * k4 );
        pk = pkm1 +  pkm2 * xk;
        qk = qkm1 +  qkm2 * xk;
...
body {
   ...
   // log(x)-x = NaN when x = real.infinity
    static immutable real MAXLOGL =  1.1356523406294143949492E4L;
...
//  if ( ax < MINLOGL ) return 0; // underflow;
    ...
    pkm2 = pkm1;
        ...
        static immutable real BIG = 9.223372036854775808e18L;
