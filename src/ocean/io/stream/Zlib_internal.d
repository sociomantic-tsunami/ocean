...
class ZlibException : IOException
{
    ...
    {
        string m = codeName(code) ~ ": ";
        m ~= StringC.toDString(msg);
        super(m);
    }

    protected string codeName(int code)
    {
        string name;
...
version (unittest)
{
    ...
    void check_array(string FILE=__FILE__, int LINE=__LINE__)(
            const(ubyte)[] as, ubyte[] bs, lazy string msg)
    {
...
unittest
{
    ...
    static immutable string message =
    ...
    // module, with the default compression level.
    {
        ...
        test( comp.written == message_z.length );
